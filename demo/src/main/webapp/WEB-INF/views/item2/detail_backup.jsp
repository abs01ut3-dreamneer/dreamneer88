...
// 		$(function(){
		document.addEventListener("DOMContentLoaded", function(){
			...
			document.addEventListener("click", function(event){
				// console.log("클릭했다!");
				// console.log("클리한 요소 : ", event.target);

				// 클릭한 요소의 클랙스가 ".clsDelete"이면
				if(event.target.matches(".clsDelete")){
					let itemId = event.target.dataset.itemId; //바닐라js dataset 사용법 기억하자
					// let itemId = $(this).data("itemId"); 차이점!!
					// console.log("check : event.target/(this).data(itemId) => ", itemId);

					let data ={
						"itemId" : itemId
					}

					//요청 URI: "/item2/deleteAjax"
					...
					.then(data=>{
						...
						//이벤트 타겟에서 가장 가까운 <div class=".card">를 없애기 (이벤트 타겟의 부모)
