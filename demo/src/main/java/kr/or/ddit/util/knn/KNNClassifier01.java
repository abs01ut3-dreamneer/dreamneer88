...
public class KNNClassifier01 {
	/**
     * 두 데이터 포인트 간의 유클리드 거리를 계산하는 헬퍼 함수
     */
   
	private static double calculateEuclidanDistance(DataPoint p1, DataPoint p2) {
     ...
     */
	public static String classify(List<DataPoint> trainingData, DataPoint newPoint, int k) throws IllegalAccessException {
		if(k<=0 || k > trainingData.size()) {
			throw new IllegalAccessException("K는 양수여야 하며 훈런 데이터의 크기보다 작거나 같아야 함");
		}
		...
		for(DataPoint dp:trainingData) {
            ...
            */
			double distance = calculateEuclidanDistance(newPoint, dp);
            /* DistancePoint
            ...
            */
			distances.add(new DistancePoint(distance, dp));
				// DistancePoint 클래스는 거리값 distance와 데이터 좌표값 dataPoint을 갖는다
