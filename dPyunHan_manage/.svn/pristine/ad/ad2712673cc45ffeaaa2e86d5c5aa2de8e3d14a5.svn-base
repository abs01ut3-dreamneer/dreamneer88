...
html, body {
	...
	overflow-x: hidden;
	overflow-y: scroll;
    scrollbar-width: none;	/* html,body영역의 스크롤을 안보이게하기. 내부 div에도 스크롤이 생겨서 적용 */
}
...
@media (max-width: 992px) {
  ...
  .side-panel.active { right: 64px; }
  .title a { max-width: 240px; }
}

@media (max-width: 640px) {
  .side-accordion { right: 2px; }  /* 모바일에서 너무 붙지 않게 */
  .side-btn {
    width: 48px;
    height: 160px;
    font-size: .9rem;
  ...
  .side-panel.active { right: 56px; }
  .title a { max-width: 58vw; }
}
		...
		<div class="list">
			<c:forEach var="bidPblancVO" items="${bidPblancVOList}">
				<div class="item">
					...
					<span class="title"><a
						href="/bidPblanc/getBidPblancList">${bidPblancVO.bidSj}</a></span> <span
						class="date"><fmt:formatDate
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  kpis.forEach(el => countIo.observe(el));
	...
	<main class="landing" id="landing">
		<section class="section" style="padding-top: 100px; align-items: start;">
			<div class="reveal">
				<h2>
					아파트 관리, <br>더 쉽고 깔끔하게.
				</h2>
