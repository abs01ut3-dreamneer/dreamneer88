...
.fc-daygrid-day-frame {
    padding: 1px 2px !important;  /* 내부 여백 줄이기 */
    min-height: 30px !important;  /* 기존 100px → 더 작게 */
}
									...
									<tr onclick="noticeDetail(${noticeVO.noticeSn})"
										...
										<td class="small text-muted mt-1"><img
											...
											alt="시계 아이콘"> <fmt:formatDate
												value="${noticeVO.noticeWritngDt}" pattern="yy-MM-dd" /></td>
										<!-- 파일 있을떄,없을때 -->
							...
							<thead>
								...
								<tr>
									...
									<td class="text-muted"><img
										...
										<span id="noticeWritngDt" style="margin-left: 4px;"></span></td>
								</tr>
								<tr>
									<th>작성자</th>
									<td>
			...
			<div class="card-header card-title">
				...
				<div class="card-tools">
					<div class="nav nav-pills ml-auto pt-1">
						<div class="nav-item ">
							<button type="button"
								onclick="location.href='/bidPblanc/postBidPblanc'"
									...
									<tr class="bidPblancVORow" style="cursor: pointer;">
										...
										<td class="text-left"><c:choose>
												...
												<c:when test="${bidPblancVO.hasAlreadyBid}">
													<span class="badge badge-warning ml-2">입찰완료</span>
												</c:when>
												<c:otherwise>
													<c:if test="${bidPblancVO.bidSttus==1}">
														<span class="badge badge-secondary ml-2">${bidPblancVO.bidSttusAsStr}</span>
													</c:if>
													<c:if test="${bidPblancVO.bidSttus!=1}">
														<span class="badge badge-info ml-2">${bidPblancVO.bidSttusAsStr}</span>
													</c:if>
												</c:otherwise>
											</c:choose> <span class="ml-2">${bidPblancVO.bidSj}</span></td>
										<td>${bidPblancVO.scsbMthAsStr}</td>
										<td class="small"><fmt:formatDate
												value="${bidPblancVO.pblancDt}" pattern="yy-MM-dd" /></td>
										<td class="small"><fmt:formatDate
												value="${bidPblancVO.bidClosDtAsDate}" pattern="yy-MM-dd" />
										</td>
								...
								<c:forEach var="CcpyManageVO"
									...
									<tr class="CcpyManageVORow" style="cursor: pointer;">
										<td class="align-middle">${CcpyManageVO.rnum }<input
											type="hidden" value="${CcpyManageVO.ccpyManageId}"
											id="ccpyManageId" name="ccpyManageId" />
										</td>
										<td class="align-middle">${CcpyManageVO.ccpyCmpnyNm }</td>
										<td class="align-middle">${CcpyManageVO.ccpyRprsntvNm }</td>
										<td class="align-middle">${CcpyManageVO.ccpyAdres }</td>
										<td class="align-middle">${CcpyManageVO.ccpyTelno }</td>
										<td class="align-middle"><fmt:formatDate
												value="${CcpyManageVO.ccpyRegistdt}" pattern="yyyy-MM-dd" />
										</td>
									</tr>
								</c:forEach>
							</tbody>
		...
		<div class="card bg-light" style="font-size: 0.75em;">
			<div class="card-header card-title">
				...
				<div class="card-tools d-flex align-items-center">
				    <button type="button"
				        onclick="location.href='/vote/addVotePage'"
				        class="btn btn-primary btn-sm mr-3 mt-1 go-link-btn">투표등록</button>
				
				    <a href="/vote/voteMtr"
				       class="btn btn-sm btn-outline-primary go-link-btn">
							...
							<c:forEach var="p" items="${articlePage1.content}"
								...
								<tr>
									...
									<td><a href="/vote/voteDetail?voteMtrSn=${p.voteMtrSn}"
										style="color: black;"> <c:out value="${p.mtrSj}" />
									</a></td>
											...
											<tr class="elctrnsanctnSentRow">
												...
												<td class="text-left"><c:choose>
														<c:when test="${elctrnsanctnVO.drftTmprstre>0}">
															<span class="badge badge-warning m1-3">임시</span>
														</c:when>
														<c:otherwise>
													...
													</c:choose> <span class="ml-2"> ${elctrnsanctnVO.drftSj } </span></td>
												<td class="text-center">${elctrnsanctnVO.sanctnEmp}</td>
												<td class="text-center"><c:choose>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='대기'}">
															<div>${elctrnsanctnVO.totSanctnStts}</div>
														</c:when>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='반려'}">
															<div style="color: red;">${elctrnsanctnVO.totSanctnStts}</div>
														</c:when>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='결재완료'}">
															<span class="badge badge-success">${elctrnsanctnVO.totSanctnStts}</span>
														</c:when>
														<c:otherwise>
															<div>${elctrnsanctnVO.totSanctnStts}</div>
														</c:otherwise>
													</c:choose></td>
												<td class="text-muted mt-1 text-center small"><fmt:formatDate
														value="${elctrnsanctnVO.drftDt}" pattern="yy-MM-dd" /></td>
												<td class="text-muted mt-1 text-center small"><fmt:formatDate
														value="${elctrnsanctnVO.creatDt}" pattern="yy-MM-dd" /></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
										<tr>
											<td colspan="7" class="text-center"><span
												class="text-muted fs-4">보낸 기안함이 비었습니다.</span></td>
										</tr>
									</c:otherwise>
								</c:choose>
							</tbody>
							...
							<tbody>
								<c:choose>
									<c:when test="${not empty articlePageElctrnsanctnRcpt.content}">
										<c:forEach var="elctrnsanctnVO"
											...
											<tr class="elctrnsanctnRcptRow">
												...
												<td class="text-center">${elctrnsanctnVO.empVO.nm }</td>
												<td class="text-center"><c:choose>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='대기'}">
															<span class="badge badge-secondary">${elctrnsanctnVO.totSanctnStts}</span>
														</c:when>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='반려'}">
															<span class="badge badge-danger">${elctrnsanctnVO.totSanctnStts}</span>
														</c:when>
														<c:when test="${elctrnsanctnVO.totSanctnStts=='결재완료'}">
															<span class="badge badge-success">${elctrnsanctnVO.totSanctnStts}</span>
														</c:when>
														<c:otherwise>
															<span class="badge badge-info">${elctrnsanctnVO.totSanctnStts}</span>
														</c:otherwise>
													</c:choose></td>
												<td class="text-center">${elctrnsanctnVO.sanctnEmp }</td>
									...
									<c:otherwise>
										<tr>
											<td colspan="7" class="text-center"><span
												class="text-muted fs-4">받은 기안함이 비었습니다.</span></td>
										</tr>
									</c:otherwise>
								</c:choose>
							</tbody>
                    ...
                    <td>해당 조건에 맞는 민원이 없습니다.</td>
                        ...
                        `;
											...
											// 종결 버튼 추가
												...
												actionBtns += `
                        <button type="button" class="btn btn-secondary btn-sm ms-1 endBtn" style="font-size: 0.5em; padding: 0.1rem 0.1rem; font-weight:700; font-size:8px; " data-cvplrceptid="\${vo.cvplRceptId}" data-cvplsn="\${vo.cvplSn}">
                            완료
                        </button>
                        `;
											}
