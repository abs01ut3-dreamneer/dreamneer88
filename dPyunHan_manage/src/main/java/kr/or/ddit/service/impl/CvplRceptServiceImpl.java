...
public class CvplRceptServiceImpl implements CvplRceptService {
	...
	public int cvplRceptPost(CvplRceptVO cvplRceptVO) {
		...
		if (uploadFiles != null) {
			if (uploadFiles[0].getOriginalFilename().length() > 0) {
				long fileGroupSn = this.uploadService.multiImageUpload(cvplRceptVO.getUploadFiles());
		...
		if (result > 0) {
			...
			// 발신자 //수신자 //알림 타입 //알림 제목
			ntcnUtil.sendAdmin(cvplRceptVO.getEmpId(), cvplVO.getMberId(), NtcnType.CVPL,
					"신청하신 \"" + cvplVO.getCvplSj() + "\" <br>민원이 접수되었습니다.",
	...
	public int cvplEnPost(CvplRceptVO cvplRceptVO) {
		...
		if (result > 0) {
			...
			// 발신자 //수신자 //알림 타입 //알림 제목
			ntcnUtil.sendAdmin(cvplRceptVO.getEmpId(), cvplVO.getMberId(), NtcnType.CVPL,
					"신청하신 \"" + cvplVO.getCvplSj() + "\" <br>민원이 종결되었습니다.",
					"/cvpl/ntcn/go?cvplSn=" + cvplRceptVO.getCvplSn());
			// 알림 URL
		}
