    ...
    <div class="container mt-4">
        ...
        <form id="residentJoinForm"
            ...
            <div class="form-step card-step">
                ...
                <div class="mb-3">
                    <label for="residesttus" class="form-label">주거타입</label> <select
                        name="residesttus" id="residesttus" class="form-control">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    carFileUpload.addEventListener("change", e => {
        Array.from(e.target.files).forEach(file => {
            readOneFile(file, carFiles, carFileList);
        });
    });
...
document.querySelector('.submit-btn').addEventListener('click', function(e){
    ...
    carFiles.forEach(file => {
        formData.append('carRegFile', file); // 'carRegFile'은 서버에서 받는 이름
    });
    */
    Swal.fire({
        icon: "success",
