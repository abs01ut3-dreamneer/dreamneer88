	...
	protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
		...
		return http.csrf(csrf->csrf.disable()).httpBasic(hb->hb.disable())
				...
				.authorizeHttpRequests(
							...
							.requestMatchers("/accessError/**","/login/**", "/user/**"
									, "/signup/**", "/js/**", "/adminlte/**"
									, "/images/**", "/css/**" ,"/main/**", "/resve/**"
									, "/ws-ntcn/**", "/.well-known/**", "/error/**", "/ntcn/**"
									, "/notifications/**"
									, "/assets/**", "/dewi/**", "/notice/listJson/**").permitAll()
							 .requestMatchers("/vote/**").hasRole("MBER")
							 .requestMatchers("/cvpl/**").hasRole("MBER")
							 .requestMatchers("/notice/**").hasRole("MBER")
							 .requestMatchers("/resve/**").hasRole("MBER")
							 .requestMatchers("/rqest/**").hasRole("MBER")
							 .requestMatchers("/visit/**").hasRole("MBER")							 
							 .requestMatchers("bbs/bbsGroupList").hasRole("MBER_HEAD")							 
				...
				.sessionManagement(session->session.maximumSessions(1))
			     .formLogin(form -> form
	                        ...
	                        .failureUrl("/login?errorMsg=INVALID_CREDENTIALS")
	                )
